...
int SyntacticalAnalyzer::Action(bool shouldReturn){
	...
	if(token == DISPLAY_T){
		...
		if(shouldReturn)
			gen->WriteCode(tabs, "return Object(");
		else
...
int SyntacticalAnalyzer::Any_Other_Token(bool addSpace){
	...
	if(token == LPAREN_T){
		...
		string code = (addSpace ? " " : "") + string("\\") + lex->GetLexeme();
		code = code.substr(0, code.length() - 1);
		code += "\\\"";
		gen->WriteCode(0, code);
		token = lex->GetToken();
	}
	else{
		gen->WriteCode(0, (addSpace ? " " : "") + lex->GetLexeme());
		if(token == IDENT_T){
...
int SyntacticalAnalyzer::Else_Part(){
	...
	int errors = 0;
    	...
    	if(token == IDENT_T
        	...
        	errors++;
        	lex->ReportError("0Unexpected token: " + lex->GetLexeme());
	   	token = lex->GetToken();
   	 }
	...
	int errors = 0;
 	//check to see if a first...
		...
		//look through to see if we have a follow....
        	...
        	lex->ReportError("1Unexpected Token; saw " + lex->GetLexeme());
        	errors++;
    	}
...
int SyntacticalAnalyzer::More_Pairs(){
	...
	int errors = 0;
    	if(token == LPAREN_T) {
        	...
        	lex->ReportError("2Unexpected Token; saw " + lex->GetLexeme());
        	errors++;
	   		token = lex->GetToken();
    	}
...
int SyntacticalAnalyzer::More_Tokens(){
	...
	errors += More_Tokens();
        ...
        errors++;
        lex->ReportError("3Unexpected Token; saw " + lex->GetLexeme());
        token = lex->GetToken();
    }
...
int SyntacticalAnalyzer::Param_List(bool needComma){
	...
	needComma = false;
        ...
        lex->ReportError("4Unexpected Token; saw " + lex->GetLexeme());
        errors++;
	    token = lex->GetToken();
    }
...
int SyntacticalAnalyzer::Stmt(bool shouldReturn){
	...
	if(token == IDENT_T){
		...
		gen->WriteCode(shouldReturn ? tabs : 0, (shouldReturn ? "return " : " ") + lex->GetLexeme() + (shouldReturn ? ";" : ""));
		token = lex->GetToken();
	}
...
int SyntacticalAnalyzer::Stmt_List(string symbol, bool printSymbol, bool shouldReturn){
	...
	lex->ReportError("5Unexpected Token; saw " + lex->GetLexeme());
        errors++;
	   token = lex->GetToken();
	}
...
int SyntacticalAnalyzer::Stmt_Pair_Body(){
	...
	if(token == LPAREN_T){
		...
		lex->ReportError("6Unexpected Token; saw " + lex->GetLexeme());
    errors++;
	  token = lex->GetToken();
	}
